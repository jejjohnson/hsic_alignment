...
class SigmaParam(NamedTuple):
    ...
    def estimate_sigma(self, X: np.ndarray, **kwargs) -> float:
        ...
        """
        if self.per_dimension == True:
            init_sigma = [
                estimate_sigma(
                    X=ifeature.reshape(-1, 1),
                    method=self.method,
                    percent=self.percent,
                    **kwargs,
                )
            ...
            init_sigma = np.array(init_sigma)
        else:
            init_sigma = estimate_sigma(
                X=X, method=self.method, percent=self.percent, **kwargs
            )
        return init_sigma
    ...
    # MEAN
        ...
        if per_dimension is True:
            ...
            if percent is None:
                ...
                sigma = [
                    np.mean(
                        np.sort(squareform(pdist(ifeature[:, None])))[:, kth_sample]
                    )
                    for ifeature in X.T
                ]
        else:
        ...
        if per_dimension is True:
            if percent is None:
                sigma = [np.median(pdist(ifeature[:, None])) for ifeature in X.T]
            else:
                kth_sample = int(percent * n_samples)
                sigma = [
                    np.median(
